...
export class WebVpnSeatClientService {
  ...
  private readonly casLoginUrl =
    process.env.SEAT_WEBVPN_CAS_URL || 'http://sso.cczu.edu.cn/sso/login';
  ...
  private async loginToGateway(
    ...
    loginUrl.searchParams.set('service', callbackUrl.toString());
    const result = await this.fetchFollowing(client, loginUrl);
    await this.submitCasForm(client, result, username, password);
