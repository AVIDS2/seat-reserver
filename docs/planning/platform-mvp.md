...
2026-09-21 完成南京工业大学图书馆公开规则核验：官方空间预约页列出阅览座位、朗读亭、面试亭三类空间；普通开馆日 7:00-8:00 预选当天座位，考研季 7:00-7:30 预选当天座位，均不是次日预约。官方规则页更新时间为 2026-03-25，公开入口为南京工业大学图书馆微信公众号，官网另有独立 `vpnlib.njtech.edu.cn` WebVPN 入口。南京工业大学已加入高校目录，接口适配与 GraphQL 座位链路已实现；不复用常州大学 `cczu` 图书馆适配器，待生产只读冒烟后开放给用户。
2026-09-21 根据脱敏 HAR 完成南京工业大学图书馆 `njtech` 适配器：独立 WebVPN/OAuth/CAS 登录、MIMO 登录验证码文字识别、GraphQL 空间目录、座位布局、当天预约和取消预约；逸夫图书馆、浦江图书馆按官方分组展示。南工大预约不使用常州大学 `freeBook` 或点选验证码，调度窗口改为北京时间 07:00；尚未使用真实账号在生产环境提交预约，部署前需完成一次只读登录与目录冒烟。
