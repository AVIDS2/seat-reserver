...
2026-09-11 增加高校工作区边界：学校账号持久化 `schoolCode`，现有账号默认归入常州大学；前端在左侧全局工作区切换器中按高校隔离账号、任务、座位图和预约记录，账号页面按高校分组。
...
2026-09-24 全局等待反馈升级：在工作台内部使用现有 Motion 和主题组件实现统一活动胶囊，账号绑定、服务连接、保存任务、提交/取消预约、读取空间目录、加载座位图和可用时段均显示可展开的处理中/完成/失败状态；登录页和落地页保持模板原有 Provider 与布局边界。
