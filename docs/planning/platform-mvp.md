...
2026-09-11 新增席定币成长闭环：成功预约按预约时长发放 10–60 席定币，运行 ID 作为唯一流水键；新增预约时长排行榜 API 和工作台页面，支持本周、本月、累计，按用户同日合并重叠区间并匿名展示。商店新增成长奖励货架，真实后端能力未完成的商品保持即将上架状态。
2026-09-11 排行榜视觉采用 MIT 许可的 Trophy Gamification UI Registry 组件思路（`leaderboard-podium`、`leaderboard-rankings`），因项目图标库和主题 token 不同而在本地适配为 Tabler/shadcn 版本；组件只负责展示，排名数据仍由平台 API 服务端计算。
