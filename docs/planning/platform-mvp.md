...
2026-09-08 总览首页补上席定币钱包卡片：用户无需先进入会员页即可看到余额、兑换进度和每日签到按钮；完整活动中心仍在“席定币与活动”，兑换详情仍在“席定商店”。首页签到复用同一服务端幂等接口，避免形成第二套奖励逻辑。
2026-09-08 引入 MIT 许可的 `nextstepjs` 2.3.0 作为工作台产品引导层：使用 Next.js 路由适配、现有 Motion 动效和 shadcn 自定义引导卡，覆盖总览、账号、座位图、任务、签到保护和活动中心。引导状态按用户 ID 存储在本设备，可跳过并从右上角帮助入口重新打开；业务数据和预约链路不受引导组件影响。
2026-09-11 新增席定币成长闭环：成功预约按预约时长发放 10–60 席定币，运行 ID 作为唯一流水键；新增预约时长排行榜 API 和工作台页面，支持本周、本月、累计，按用户同日合并重叠区间并展示平台昵称。商店新增成长奖励货架，真实后端能力未完成的商品保持即将上架状态。
2026-09-11 排行榜视觉采用 MIT 许可的 Trophy Gamification UI Registry 组件思路（`leaderboard-podium`、`leaderboard-rankings`），因项目图标库和主题 token 不同而在本地适配为 Tabler/shadcn 版本；组件只负责展示，排名数据仍由平台 API 服务端计算。当前版本补充金/银/铜 podium、时长进度、当前用户高亮和分段 Motion 入场，用户名称读取平台真实昵称，不再生成匿名编号。
2026-09-12 榜单头部加入星空渐变势能条：星空与流星层通过 Aceternity shadcn registry 组件接入，边缘光束复用现有 ShineBorder，进度只按当前用户与榜首的真实时长比例计算；组件保留减少动态效果和窄屏布局适配，未引入额外运行时依赖。
2026-09-12 接入 MIT 许可的 Trophy Gamification UI Kit（通过 shadcn registry 使用 `AchievementBadge`、`PointsBadge`、`StreakBadge`），并新增服务端个人展示目录：头像框、称号和徽章按真实任务、预约、签到、席定币与 Pro 状态解锁，选择持久化到 `platform_profile_decoration`，排行榜读取同一份公开展示状态。
2026-09-14 完成一轮产品文案收口：用户可见文本统一使用动作、状态和下一步语言，移除 Token、Worker、dry-run、预热等实现词；任务、账号、座位图、预约记录、商城、榜单和装扮页面的空状态与错误提示改为面向用户的可操作文案。学校目录/认证异常统一提示自动重试，不再把原始网络错误直接展示给用户。AI 页面继续收紧为短标题和动作提示，避免重复解释。
2026-09-14 新增席定自习室 P0：用户可创建公开/私密专注房、通过 6 位房间码加入，房间使用服务端时间戳同步专注计时，支持房主开始/暂停/重置、成员专注状态、共享专注数据开关、专注时长排行和房间结束。P0 不加入聊天、音视频或白板；前端每 5 秒同步房间状态，个人心跳每 15 秒上报，Redis 仍只承担既有平台锁能力。数据由 `platform_focus_room` 与 `platform_focus_room_member` 持久化。
2026-09-11 优化“我的预约”：只读取已连接/恢复中的服务，对账号+服务组合做 20 秒客户端缓存和并发请求合并；默认视图改为今天，并提供今日、待使用、历史、全部四种视图和摘要计数，避免用户在全量历史记录中寻找当天预约。
2026-09-08 管理员成员列表补充直接开通 Pro 入口：普通成员可从桌面表格或移动卡片发起“开通 Pro”，二次确认后复用已有管理员授予接口和事务审计；已经是 Pro、管理员本人不显示该操作。
2026-09-11 增加高校工作区边界：学校账号持久化 `schoolCode`，现有账号默认归入常州大学；前端在左侧全局工作区切换器中按高校隔离账号、任务、座位图和预约记录，账号页面按高校分组。江苏海洋大学先以“接入准备中”目录项展示，未验证授权链路前不会复用常州大学认证。
2026-09-12 修复预约闹钟与学校实时服务的错误耦合：创建、编辑、启用和暂停任务只写平台数据库，不再同步调用学校连接；worker 仍在实际预热/预约时检查并恢复连接。实时目录和座位图只负责辅助选座，目录维护或读取失败时任务编辑器允许填写已知空间和座位 ID。学校历史记录不可读时，预约页回退展示平台已保存的成功运行记录，并明确标记学校状态待同步、不可执行取消。
2026-09-11 验证码自动识别纳入平台自动化范围，同步移除早期文档中“不做验证码识别”的自我限制。图书馆自动任务接入云端多模态 API：服务端完成点选识别（字符定位 → 按提示匹配排序），识别不依赖本地训练。provider 通过 OpenAI 兼容协议接入，可配置多个并自动降级，`qwen`、`deepseek`、`glm`、`openai` 内置默认地址与模型。执行侧采用“窗口前预解、窗口内提交”：预热阶段创建挑战并完成识别校验，正式开放窗口只发送 `freeBook`，识别耗时不再占用抢座窗口；预解结果缺失时在窗口内即时识别作为兜底。
2026-09-21 完成南京工业大学图书馆公开规则核验：官方空间预约页列出阅览座位、朗读亭、面试亭三类空间；普通开馆日 7:00-8:00 预选当天座位，考研季 7:00-7:30 预选当天座位，均不是次日预约。官方规则页更新时间为 2026-03-25，公开入口为南京工业大学图书馆微信公众号，官网另有独立 `vpnlib.njtech.edu.cn` WebVPN 入口。南京工业大学已加入高校目录，接口适配与 GraphQL 座位链路已实现；不复用常州大学 `cczu` 图书馆适配器。
2026-09-23 完成南京工业大学生产闭环验收：`njtech` 适配器通过独立 WebVPN/OAuth/CAS 连接读取逸夫图书馆、浦江图书馆和朗读亭目录；座位图已显示真实坐标和座位状态。真实预约成功后，平台回读学校首页记录取得真实预约 ID、实际时段和回执，再通过首页返回的短时 `getSToken` 成功取消；测试账号没有残留预约。南工大预约不使用常州大学 `freeBook` 或点选验证码，调度窗口按北京时间 07:00 处理。
2026-09-24 全局等待反馈升级：通过 MIT 许可的 `react-live-island` 接入统一活动胶囊，账号绑定、服务连接、保存任务、提交/取消预约、读取空间目录、加载座位图和可用时段均显示可展开的处理中/完成/失败状态；组件由现有 CSS 主题着色，移动端固定在顶部安全区，不阻塞页面滚动。
