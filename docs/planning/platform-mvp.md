...
2026-09-14 新增番茄自习室 P0：用户可创建公开/私密专注房、通过 6 位房间码加入，房间使用服务端时间戳同步番茄钟，支持房主开始/暂停/重置、成员专注状态、共享专注数据开关、专注时长排行和房间结束。P0 不加入聊天、音视频或白板；前端每 5 秒同步房间状态，个人心跳每 15 秒上报，Redis 仍只承担既有平台锁能力。数据由 `platform_focus_room` 与 `platform_focus_room_member` 持久化。
