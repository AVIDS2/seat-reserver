...
- “朗读亭”沿用南工大 GraphQL 座位服务，但按上午、下午、晚上三个时段分别调度；“面试亭”另建服务类型，需要处理“e 面试”小程序和电子锁密码，不能先混入座位图任务。
...
1. 预选窗口固定从 7:00 开始；普通季与考研季的实时开馆起点由官方规则和 GraphQL 的 `open_time_str` 返回值共同决定，平台不把 8:00 写死为南工大所有季节的使用起点。
