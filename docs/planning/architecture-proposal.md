...
2026-09-03 起，校园账号只保存身份凭据，每套预约系统通过 `platform_school_service_connection` 维护独立的业务 Token、认证模式和验证时间。自习室与图书馆共用统一目录适配层，对前端输出馆区/楼栋、空间、日期、座位图和时段；任务保存真实展示名称与内部 ID，并支持每天、工作日、自定义星期或指定日期调度。长表单的标题、滚动内容和操作区分层固定，避免移动端弹层打开下拉选项时发生视口跳动。图书馆当前开启验证码，只开放实时浏览和暂停任务，待验证码确认接口完成后再开放提交与自动调度。
