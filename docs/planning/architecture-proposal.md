...
2026-09-14 新增线上番茄自习室：专注房间与预约任务分域，房间状态和成员专注累计写入 PostgreSQL，服务端保存当前阶段结束时间，前端按时间戳本地倒计时并定期心跳。P0 不新增 WebSocket 服务，避免为安静陪伴场景引入聊天/音视频基础设施；房间规模上限 20 人，Redis 仅继续用于平台既有锁。后续规模需要更低延迟时，可在同一 REST 契约上增加 SSE/WebSocket 推送。
