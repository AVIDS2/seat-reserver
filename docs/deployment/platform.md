...
2026-09-04 的故障排查确认：API 重启会清空 WebVPN 的进程内运行对象，但现在会把加密后的 Cookie、代理上下文和过期时间保存到服务连接，并在重启后按需恢复；恢复失败才重新登录。VPS 可以访问 `zmvpn.cczu.edu.cn`，但到 `sso.cczu.edu.cn` 的公网连接当前超时，因此图书馆和处于 WebVPN 模式的账号仍需要学校统一认证链路可达且凭据有效；自习室的 `direct` 账号不依赖该 SSO 回退。平台会保留学校明确返回的账号密码错误，不再无条件重复尝试，以免消耗学校登录次数。
...
- Worker 先按账号认证模式验证缓存 Token；webvpn 模式会先恢复加密的网关 Cookie、校园 SSO、代理入口和签名上下文，失效时再用已加密保存的学校账号密码重新登录。当前实现不创建系统级 VPN，也不改变 VPS 全局路由。
