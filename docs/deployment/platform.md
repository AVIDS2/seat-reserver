...
7. 在“预约任务”选择预约系统、实时馆区/楼栋、空间、日期、座位图和时间；首个座位为主座位，其余为有序备选。
8. 在“执行频率”选择每天、工作日、自定义星期或单次。每天/工作日/自定义星期不绑定固定目标日期，日期控件用于查看学校当前可预约日期；单次模式同时把所选日期保存为执行日期。
...
任务里的座位号用于人类可读展示，系统座位 ID 只在后端提交预约时使用。控制台已经接入学校实时目录与布局，用户不再手填系统 ID；馆区/楼栋、空间、日期和占用状态随服务实时刷新。任务支持“每天、工作日、自定义星期、单次”，单次任务成功后自动停用。

移动端弹窗统一使用稳定的 `svh` 高度、标题固定区、独立滚动区和底部固定操作区；Select/Popover 的内容限制在视口宽度内并使用 `overscroll-contain`，全局保留滚动条槽，防止打开控件时页面横向或纵向跳动。管理员工作台的标签栏在手机端局部吸顶，运行记录、账号编辑和邀请码弹窗也遵循同一容器规则。
