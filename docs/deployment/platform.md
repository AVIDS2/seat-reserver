...
学校账号绑定支持两种模式并自动识别：已有一考即过凭据走 `direct`；普通校园账号在 direct 失败后自动走 `zmvpn → CAS → ssoAuth` 的 `webvpn` 链路。现有 direct 账号迁移后保持原模式，不改变每天的公网预热和预约。
...
当前平台已经固化 direct 和 webvpn 两条已验证认证链路。WebVPN 代理哈希、CAS 临时 JWT 和座位签名种子均在运行时动态获取。不要把真实 HAR、学校密码、Token 或 HMAC key 提交到 Git。
