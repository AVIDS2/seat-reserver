...
当前限制：新账号从校园统一身份认证、验证码或激活流程首次换取一考即过内部凭据的链路尚未完成真实抓包验收。现有表单不能保证“校园学号 + 校园密码”可直接绑定；不要把老账号可自动续期误认为新账号首次接入已经闭环。
...
当前平台已经固化已验证的预约请求头、老账号 token 刷新和接口路径；新账号首次 SSO/验证码/绑定链路仍需采集。不要把真实 HAR、学校密码、Token 或 HMAC key 提交到 Git。
