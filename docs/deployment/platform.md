...
6. 后端先尝试 `/rest/auth` 和 `/rest/v2/user`；不接受该凭据时自动切换 WebVPN 网关并完成同样的 Token 验证，验证成功后才保存账号。
...
学校账号绑定支持两种模式并自动识别：已有一考即过凭据走 `direct`；普通校园账号在 direct 失败后自动走 `zmvpn` 网关登录、动态发现座位应用代理并交换 `ssoAuth` 的 `webvpn` 链路。现有 direct 账号迁移后保持原模式，不改变每天的公网预热和预约。
...
2026-09-02 的一次生产测试已确认任务会真实进入学校预约接口，但学校返回 HTTP 200、业务码 `1`、消息“验证码错误”，运行记录为失败且没有预约回执；因此“账号 Token 可验证”不等于“预约链路已成功”，验收必须以运行记录的成功状态和回执为准。
...
当前平台已经固化 direct 和 webvpn 两条认证路由。WebVPN 登录字段、代理地址、座位临时授权票据和签名种子均在运行时动态获取；VPS 上的网关登录已验证，完整 Token/预约链路仍需以生产测试账号完成最终验收。不要把真实 HAR、学校密码、Token 或 HMAC key 提交到 Git。
