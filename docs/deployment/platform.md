...
平台按服务选择认证路线：自习室使用公网 `cczukaoyan` 直连 Token，图书馆使用 `zmvpn` 网关和校园 SSO。新增自习室账号默认走直连；历史上被错误标成 WebVPN 的自习室连接会在下一次访问时重新获取直连 Token。图书馆登录后，平台从 CAS 最终回跳地址动态提取对应的代理入口，再交换 `ssoAuth` 业务 Token；用户端始终只需填写学号和密码。

WebVPN 网关登录会先读取当前登录页下发的会话密钥，再按网关页面的 AES 参数提交账号密码；登录后的 `clientInfo` Cookie 按 `/enlink/` 路径读取，以兼容网关更新后的 Cookie 作用域。

已有学校账号恢复连接时按服务使用固定模式：自习室始终刷新公网直连 Token，图书馆始终重建 WebVPN 会话；首次添加账号仍会执行直连优先的自动探测，历史上被标成 WebVPN 的自习室连接会在下一次访问时自动迁移到直连。

2026-09-04 的故障排查确认：API 重启会清空 WebVPN 的进程内运行对象，但现在会把加密后的 Cookie、代理上下文和过期时间保存到服务连接，并在重启后按需恢复；恢复失败才重新登录。VPS 可以访问 `zmvpn.cczu.edu.cn`，但到 `sso.cczu.edu.cn` 的公网连接当前超时，因此图书馆连接仍需要学校统一认证链路可达且凭据有效；自习室直连不依赖 WebVPN。平台会保留学校明确返回的账号密码错误，不再无条件重复尝试，以免消耗学校登录次数。
...
- Worker 先按服务认证模式验证缓存 Token；自习室只刷新公网直连 Token，图书馆先恢复加密的网关 Cookie、校园 SSO、代理入口和签名上下文，失效时再用已加密保存的学校账号密码重新登录。当前实现不创建系统级 VPN，也不改变 VPS 全局路由。
...
2026-09-03 曾使用生产测试账号完成 WebVPN 兼容链路验收：平台自动登录、交换和验证 Token，从实时布局筛出可用座位，并在第一次尝试成功预约 5 号楼 1 层智能自习室 148 号，时间 16:00-17:00。当前自习室主路线已切回公网 direct；学校接口返回 HTTP 200、业务码 `0`、真实回执；临时平台任务随后删除，学校预约保留。
...
当前平台已经固化服务级认证路由：自习室使用 direct，图书馆使用 webvpn。WebVPN 登录字段、CAS 票据、代理地址、业务 Token 和签名种子均在运行时动态获取。不要把真实 HAR、学校密码、Token 或 HMAC key 提交到 Git。
