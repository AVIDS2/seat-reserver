...
南京工业大学使用独立的 `njtech` 适配器：WebVPN/CAS 登录后进入 `seat.njtech.edu.cn` 的 GraphQL 座位服务。`NJTECH_WEBVPN_ENTRY_URL` 留空时，服务端从 WebVPN 统一入口解析 OAuth、CAS 和座位服务；只有学校调整入口时才需要覆盖该变量。当前支持逸夫图书馆、浦江图书馆的当天座位抢座，预约窗口为 7:00，考研季窗口由学校返回的开放配置决定。座位查询使用 `list`/`libLayout`，预约使用 `reserueSeat`，取消使用 `reserveCancle`。南工大登录验证码由已配置的 MIMO 文字识别 provider 读取，座位提交本身不复用常州大学点选验证码。
