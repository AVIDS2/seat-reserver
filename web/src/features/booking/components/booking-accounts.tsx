  ...
  return (
    <PageContainer>
      <div className='mx-auto flex w-full max-w-[1200px] flex-col gap-5 sm:gap-6'>
        <div className='flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'><div className='min-w-0'><p className='text-muted-foreground mb-2 text-sm'>连接管理</p><h1 className='text-2xl font-semibold tracking-tight sm:text-3xl'>账号与授权</h1><p className='text-muted-foreground mt-2 text-sm leading-6'>学校凭据只在后端加密保存，用于正常登录、Token 验证和预约。</p></div><Button className='w-full sm:w-auto' variant='outline' onClick={() => { setEditorAccount(undefined); setEditorOpen(true); }}><Icons.add data-icon='inline-start' />添加账号</Button></div>
        ...
        {accounts.length === 0 ? <Card className='shadow-none'><CardContent className='flex flex-col items-center justify-center gap-2 py-16 text-center'><Icons.shield className='text-muted-foreground/40 size-10' /><p className='text-sm font-medium'>还没有学校账号</p><p className='text-muted-foreground text-xs'>添加并验证账号后，才能创建自动预约任务。</p></CardContent></Card> : <div className='grid gap-4 lg:grid-cols-2'>{accounts.map((account) => { const isRefreshing = refreshingId === account.id; const connected = account.status === 'connected'; return <Card key={account.id} className='min-w-0 shadow-none'><CardHeader className='border-b'><div className='flex min-w-0 items-start gap-3'><div className='bg-muted flex size-10 shrink-0 items-center justify-center rounded-lg'><Icons.user className='size-5' /></div><div className='min-w-0'><CardTitle className='truncate text-lg'>{account.label}</CardTitle><CardDescription className='mt-1 truncate'>{account.username}</CardDescription></div></div><Badge variant='outline' className={cn(connected ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' : 'border-amber-500/20 bg-amber-500/10 text-amber-700 dark:text-amber-400')}><Icons.circleCheck />{account.statusLabel}</Badge></CardHeader><CardContent className='flex flex-col gap-5 pt-5'><div className='grid grid-cols-2 gap-4'><Info label='授权状态' value={account.tokenLabel} /><Info label='关联任务' value={`${account.tasks} 个任务`} /><Info label='最近刷新' value={account.refreshedAt} /><Info label='最近验证' value={account.lastVerifiedAt} /></div><div className='flex flex-wrap justify-end gap-2 border-t pt-4'><Button variant='ghost' size='sm' onClick={() => { setEditorAccount(account); setEditorOpen(true); }}><Icons.edit data-icon='inline-start' />编辑</Button><Button variant='ghost' size='sm' className='text-destructive' onClick={() => setDeleteTarget(account)}><Icons.trash data-icon='inline-start' />移除</Button><Button variant='outline' size='sm' onClick={() => void refreshAccount(account)} disabled={isRefreshing}><Icons.refresh className={cn(isRefreshing && 'animate-spin')} />{isRefreshing ? '刷新中' : '刷新 Token'}</Button></div></CardContent></Card>; })}</div>}
      </div>
