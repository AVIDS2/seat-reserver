  ...
  useEffect(() => {
    ...
    void getLeaderboardSnapshot(
      ...
      .catch((error) => {
        if (active)
          toast.error(
            error instanceof Error ? error.message : "排行榜加载失败",
          );
      })
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-[1080px] flex-col gap-4 sm:gap-5">
        ...
        >
          <Card className="relative overflow-hidden border-amber-400/40 bg-gradient-to-br from-amber-400/15 via-card to-sky-400/10 shadow-none">
            <CardContent className="flex flex-col gap-6 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
              <div className="min-w-0">
                ...
                >
                  <Icons.trophy data-icon="inline-start" /> 席定学习榜
                </Badge>
              ...
              <div className="flex shrink-0 items-center gap-4 self-start sm:self-auto">
                <motion.div
                  className="flex size-16 items-center justify-center rounded-2xl bg-amber-400 text-amber-950 shadow-sm"
                ...
                <div>
                  <p className="text-muted-foreground text-xs">当前榜单</p>
