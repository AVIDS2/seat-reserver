...
export default function FocusRoomPage({ initialRoom }: { initialRoom: FocusRoom }) {
  ...
  const remaining = useMemo(() => getRemainingSeconds(room, now + serverOffset), [now, room, serverOffset]);
  ...
  const runTimerAction = async (action: 'start' | 'pause' | 'reset') => {
    ...
    try {
      ...
      toast.success(action === 'start' ? '专注开始' : action === 'pause' ? '计时已暂停' : '计时已重置');
    } catch (error) {
  ...
  return (
    ...
    >
      <div className='mx-auto flex w-full max-w-[1180px] flex-col gap-5 sm:gap-6'>
          ...
          <div className='relative flex flex-col gap-5 p-5 sm:p-7'>
            <div className='flex flex-wrap items-start justify-between gap-4'>
              <div className='min-w-0'>
                ...
                <h1 className='mt-4 truncate text-2xl font-semibold tracking-tight sm:text-4xl'>{room.name}</h1>
                <p className='mt-2 text-sm text-background/60'>房主：{room.hostName} · {room.memberCount}/{room.maxMembers} 人</p>
              </div>
              <button
                type='button'
            ...
            <div className='grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-end'>
              <div>
                ...
                <p className='mt-2 text-sm text-background/75'>{timerLabel(room.timer.status)} · 已完成 {room.timer.completedRounds} 轮</p>
                <p className='text-muted-foreground mt-4 text-xs'>房主控制房间节奏，你可以单独标记自己的专注状态。</p>
              </div>
              <FocusTimerRing
                progress={progress}
              ...
              >
                <div className='flex items-end gap-2' role='timer' aria-live='polite'>
                  <TimerDigit value={formatDuration(remaining).slice(0, 2)} label='分钟' reduceMotion={reduceMotion} />
                  <span className='pb-7 font-mono text-3xl text-primary/75'>:</span>
                  <TimerDigit value={formatDuration(remaining).slice(3)} label='秒' reduceMotion={reduceMotion} />
                </div>
              </FocusTimerRing>
            </div>
            {room.isHost && (
              <div className='flex flex-wrap gap-2'>
                <Button onClick={() => void runTimerAction('start')} disabled={busy !== null || room.timer.status === 'running'}>
                  <Icons.play data-icon='inline-start' /> 开始
                </Button>
                <Button variant='secondary' onClick={() => void runTimerAction('pause')} disabled={busy !== null || room.timer.status !== 'running'}>
                  <Icons.pause data-icon='inline-start' /> 暂停
                </Button>
                <Button variant='ghost' className='text-background hover:bg-background/10 hover:text-background' onClick={() => void runTimerAction('reset')} disabled={busy !== null}>
        ...
        <div className='grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.72fr)]'>
          <Card className='shadow-none'>
            <CardHeader className='border-b'>
              <div className='flex items-start justify-between gap-3'>
                <div>
                  ...
                  <CardTitle className='mt-1 flex items-center gap-2 text-xl'><Icons.teams /> 房间成员</CardTitle>
                </div>
            ...
            <CardContent className='flex flex-col gap-2 pt-4'>
              {room.members.map((member, index) => (
                <MemberRow key={member.id} member={member} rank={index + 1} currentUserId={room.currentUser?.userId} shareFocusData={room.shareFocusData} />
              ))}
            </CardContent>
          </Card>
          ...
          <div className='flex flex-col gap-5'>
              ...
              <Card className='shadow-none'>
                ...
                <CardContent className='flex flex-col gap-4 pt-5'>
                  <div className='flex items-center justify-between gap-4 rounded-lg bg-muted/40 p-3'>
                    <div>
                      <p className='text-sm font-medium'>{room.currentUser?.isFocused ? '我正在专注' : '我还没有开始'}</p>
                      <p className='text-muted-foreground mt-1 text-xs'>累计 {room.currentUser?.focusMinutesLabel || '0 分钟'}</p>
                    </div>
                    <Switch checked={Boolean(room.currentUser?.isFocused)} onCheckedChange={(checked) => void toggleFocus(checked)} disabled={busy !== null} aria-label='切换个人专注状态' />
                  </div>
                  <p className='text-muted-foreground text-xs leading-5'>房间计时由房主控制，你可以独立标记自己的专注状态。</p>
                </CardContent>
              </Card>
            )}
            <Card className='shadow-none'>
            ...
            <div className='flex flex-wrap gap-2'>
              {room.isHost ? (
                <Button variant='outline' className='text-destructive' onClick={() => setConfirmAction('close')} disabled={busy !== null}>
                  <Icons.close data-icon='inline-start' /> 结束房间
                </Button>
              ) : room.isMember ? (
                <Button variant='outline' className='text-destructive' onClick={() => setConfirmAction('leave')} disabled={busy !== null}>
      ...
      <AlertDialog open={confirmAction !== null} onOpenChange={(open) => !open && setConfirmAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{confirmAction === 'close' ? '结束这个房间？' : '离开这个房间？'}</AlertDialogTitle>
            <AlertDialogDescription>
              {confirmAction === 'close' ? '结束后房间码失效，成员将无法继续加入。' : '离开后不会删除你的专注记录，之后仍可凭房间码重新加入。'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function TimerDigit({ value, label, reduceMotion }: { value: string; label: string; reduceMotion: boolean | null }) {
  return (
    <div className='flex flex-col gap-1'>
      <motion.span key={value} initial={reduceMotion ? false : { opacity: 0.4, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.16 }} className='font-mono text-6xl font-semibold leading-none tabular-nums text-background sm:text-8xl'>
        {value}
      </motion.span>
...
function MemberRow({ member, rank, currentUserId, shareFocusData }: { member: FocusRoomMember; rank: number; currentUserId?: string; shareFocusData: boolean }) {
  const online = Date.now() - new Date(member.lastSeenAt).getTime() <= 60_000;
  const isCurrent = member.userId === currentUserId;
  return (
    <div className={cn('flex min-w-0 items-center gap-3 rounded-lg border p-3', isCurrent && 'border-primary/40 bg-primary/[0.04]')}>
      <span className='text-muted-foreground w-6 shrink-0 text-center font-mono text-xs tabular-nums'>#{rank}</span>
      <ProfileAvatar avatarUrl={member.avatarUrl} name={member.displayName} size='default' showStatus={online} />
      ...
      <div className='shrink-0 text-right'>
        ...
        <p className={cn('mt-1 text-xs', member.isFocused ? 'text-primary' : 'text-muted-foreground')}>{member.isFocused === null ? (shareFocusData ? '离线' : '仅本人可见') : member.isFocused ? '专注中' : '休息中'}</p>
      </div>
    </div>
  );
}
...
function Setting({ label, value }: { label: string; value: string }) {
  return <div className='rounded-lg bg-muted/40 p-3'><p className='text-muted-foreground text-xs'>{label}</p><p className='mt-1 font-medium tabular-nums'>{value}</p></div>;
}

function getRemainingSeconds(room: FocusRoom, now: number): number {
  if (room.timer.status === 'running' && room.timer.phaseEndsAt) return Math.max(0, Math.ceil((new Date(room.timer.phaseEndsAt).getTime() - now) / 1000));
