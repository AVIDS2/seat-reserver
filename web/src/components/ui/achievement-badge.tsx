...
const AchievementBadge = React.forwardRef<HTMLDivElement, AchievementBadgeProps>(
  (
    ...
    const isUnlocked = achievement.unlocked ?? achievement.achievedAt !== null;
    ...
    const statusLabel = isUnlocked ? '已解锁' : '未解锁';
    const itemLabel = `${achievement.name} - ${statusLabel}`;
    const interactionProps = onAchievementClick
      ? {
      ...
      >
        ...
        >
          ...
          {achievement.badgeUrl ? (
            ...
            >
              <BadgeIcon className={cn(iconSizeMap[badgeSize], iconClassName)} />
            </div>
          )}
        </div>
