...
interface StreakBadgeProps
  extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof streakBadgeVariants> {
  /** Streak length value */
  length?: number;
...
const StreakBadge = React.forwardRef<HTMLDivElement, StreakBadgeProps>(
  ({ className, size, length, frequency = 'daily', subtitle, unitLabel, icon, ...props }, ref) => {
    ...
    const valueUnit = unitLabel ?? pluralLabel;

    // Build accessible label
    const ariaLabel = `${streakLength} ${valueUnit} ${subtitleText}`;
    ...
    return (
      <div
        ...
        {...props}
      >
        {icon ?? <Flame className={cn(iconSize, 'text-primary shrink-0')} aria-hidden='true' />}
