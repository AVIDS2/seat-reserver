...
/* oxlint-disable -- generated production UI from assistant-ui registry. */
/* eslint-disable */
...
} from '@/components/assistant-ui/attachment.aui';
import { File } from '@/components/assistant-ui/file';
import { ThreadFollowupSuggestions } from '@/components/assistant-ui/follow-up-suggestions.aui';
...
import {
  ...
  ReasoningText,
  ReasoningTrigger
} from '@/components/assistant-ui/reasoning.aui';
import { ToolFallback } from '@/components/assistant-ui/tool-fallback.aui';
import {
  ToolGroupContent,
  ToolGroupRoot,
  ToolGroupTrigger
} from '@/components/assistant-ui/tool-group.aui';
  ...
  return (
    ...
    >
      <ThreadPrimitive.Viewport
        ...
        className='relative flex flex-1 touch-pan-y flex-col overflow-x-hidden overflow-y-auto overscroll-contain scroll-smooth'
      >
        <div
          className={cn(
...
const ThreadMessage: FC = () => {
  ...
  const isSpoken = useAuiState(
    (s) => (s.message.metadata as { modality?: string } | undefined)?.modality === 'voice'
  );
...
const useVoiceRunPosition = (): VoiceRunPosition =>
  useAuiState((s) => {
    const before =
      (s.thread.messages[s.message.index - 1]?.metadata as { modality?: string } | undefined)
        ?.modality === 'voice';
    const after =
      (s.thread.messages[s.message.index + 1]?.metadata as { modality?: string } | undefined)
        ?.modality === 'voice';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className='sr-only'>{role === 'user' ? '你说' : '席定领航说'}</span>
...
const SpokenActionBar: FC = () => {
  return (
    ...
    >
      <ActionBarPrimitive.Copy render={<TooltipIconButton tooltip='复制' className='size-6' />}>
...
const AssistantActionBar: FC = () => {
  return (
    ...
    >
      ...
      <ActionBarPrimitive.Reload render={<TooltipIconButton tooltip='重新生成' />}>
        <RefreshCwIcon />
      </ActionBarPrimitive.Reload>
      <ActionBarMorePrimitive.Root>
        <ActionBarMorePrimitive.Trigger
          render={<TooltipIconButton tooltip='更多' className='data-[state=open]:bg-accent' />}
        >
          <MoreHorizontalIcon />
        </ActionBarMorePrimitive.Trigger>
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Edit
        render={<TooltipIconButton tooltip='编辑' className='aui-user-action-edit' />}
      >
        <PencilIcon />
      </ActionBarPrimitive.Edit>
    </ActionBarPrimitive.Root>
...
const BranchPicker: FC<BranchPickerPrimitive.Root.Props> = ({ className, ...rest }) => {
  return (
    <BranchPickerPrimitive.Root
      ...
      {...rest}
    >
      <BranchPickerPrimitive.Previous render={<TooltipIconButton tooltip='上一条' />}>
        <ChevronLeftIcon />
      </BranchPickerPrimitive.Previous>
      <span className='aui-branch-picker-state font-medium'>
        <BranchPickerPrimitive.Number /> / <BranchPickerPrimitive.Count />
      </span>
      <BranchPickerPrimitive.Next render={<TooltipIconButton tooltip='下一条' />}>
        <ChevronRightIcon />
